    ...
    return (
      <div className="bg-green-50/50 rounded-[3rem] p-12 border border-green-100/50 relative overflow-hidden shadow-inner text-center animate-in fade-in zoom-in duration-500">
        ...
        <button 
          ...
          className="mt-8 text-xs font-black text-green-900/60 uppercase tracking-widest hover:text-green-900 transition-colors"
        >
          ลงทะเบียนอีเมลอื่น
        </button>
      </div>
    ...
    <div className="bg-blue-50/50 rounded-[3rem] p-12 border border-blue-100/50 relative overflow-hidden shadow-inner">
      ...
      <form onSubmit={handleSubscribe} className="space-y-3">
        ...
        <button 
          type="submit"
          disabled={status === 'loading'}
          className="w-full bg-[#0f3460] text-white py-4 rounded-xl font-black text-xs uppercase tracking-[0.2em] nav-font shadow-lg shadow-blue-100 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
        >
          {status === 'loading' ? (
            <>
