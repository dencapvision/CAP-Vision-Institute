...
DROP POLICY IF EXISTS "Service role full access on ai_chat_logs" ON ai_chat_logs;
CREATE POLICY "Service role full access on ai_chat_logs"
  ON ai_chat_logs FOR ALL
  USING (true)
  WITH CHECK (true);

DROP POLICY IF EXISTS "Service role full access on ai_sessions" ON ai_sessions;
